...
def l0_phasing(
    ...
    *,
    ...
    if abs(scandata["psi_meas"]) < 20.0:
        ...
        apply_correction = (
            input(
                f"Measured psi = {scandata['psi_meas']}. Update REFPOC? [y/n]"
            ).lower()
            == "y"
        )
    if apply_correction:
...
def fast_phase_scan_l0(env, k, p0, pf, Nshots):
    ...
    buf = f2BeamSynchronousBuffer(
        EPICS_address_list=[
            ...
            "BPMS:IN10:731:X",  # DL10 energy BPM
            "BPMS:IN10:731:TMIT",  # for data sanitization
        ],
        EPICS_Npts=Nshots,
        verbose=True,
        nowait=True,
...
def plot_scan_result(scandata):
    ...
    _fit_phi = np.linspace(
        min(scandata["phi"]),
        max(scandata["phi"]),
        200,
    )
    _fit_X = (
        scandata["A_meas"] * np.cos(np.deg2rad(_fit_phi) - scandata["psi_meas"])
        + scandata["B_meas"]
    )
