...
_NUISANCE_HIGH = [10.0, 0.9, 15.0, 100.0, 10.0, 0.01, 0.05, 3.0, 0.4, 45.0]
...
class TwoBunchDoubleExp:
    ...
    def __call__(self, xi, thetas, nuisance, noiseless=False):
        ...
        n_xi = xi.shape[0]
        if A.shape[0] != n_xi:
            A, f1, tau1, tau2, tau_kick = [
                v.repeat(n_xi, 1) for v in (A, f1, tau1, tau2, tau_kick)
            ]
            sigma, y_shift, irf_sigma = [
                v.repeat(n_xi, 1) for v in (sigma, y_shift, irf_sigma)
            ]
            alpha_kick, delta_T, thetas = [
                v.repeat(n_xi, 1) for v in (alpha_kick, delta_T, thetas)
            ]
        ...
        def exp_component(amp, tau_i, dt):
            ...
            b1 = (
                (amp / 2)
                * torch.exp(-(dt**2) / (2 * s**2))
                * torch.special.erfcx(q.clamp(min=0))
            )
            b2 = (
                (amp / 2)
                * torch.exp((s**2 / (2 * tau_i**2) - dt / tau_i).clamp(max=0))
                * torch.special.erfc(q.clamp(max=0))
            )
...
class AmortizedBOEDBunchGenerator(Generator):
    ...
    def validate_vocs(cls, v, info: ValidationInfo):
        ...
        if not v.n_objectives == 0:
            raise VOCSError(
                "AmortizedBOEDBunchGenerator generator only supports problems with no objectives"
            )
    ...
    def __init__(self, model_dir, design_range, observable_name, vocs=None, **kwargs):
        ...
        round_models = {}
        for p in sorted(
            model_dir.glob("posterior_sampling_traced_round_*_horizon_*.pt")
        ):
    ...
    def _load_model(self, r):
        if r not in self._model_cache:
            model = torch.jit.load(
                str(self._round_to_path[r]), map_location=self.device
            )
            model.eval()
    ...
    def generate(self, n_candidates=1):
        ...
        if self._normalizer is None:
            ...
            print(
                f"Normalization auto-fitted: y_min={self._normalizer.y_min:.4f}  y_max={self._normalizer.y_max:.4f}"
            )

        norm = self._normalizer
        t_phys = torch.tensor(self.data[var].values, dtype=torch.float32).unsqueeze(1)
        y_phys = torch.tensor(
            self.data[self._observable].values, dtype=torch.float32
        ).unsqueeze(1)
        ...
        samples_norm = (
            means[0, idx, :]
            + stds[0, idx, :] * torch.randn(n_candidates, means.shape[-1])
        )[:, 0].clamp(0.0, 1.0)
...
def get_results(X, grid_steps, settings):
    ...
    with torch.no_grad():
        means, stds, log_weights = X.generator._load_model(last_round)(
    ...
    samples_norm = (
        means[0, idx, :]
        + stds[0, idx, :]
        * torch.randn(settings["n_posterior_samples"], means.shape[-1])
    )[:, 0].clamp(0.0, 1.0)
    ...
    if settings["visualize"]:
        ...
        lo68, hi68 = (
            float(np.percentile(t0_samples, 16)),
            float(np.percentile(t0_samples, 84)),
        )
        ax.hist(t0_samples, bins=60, density=True, color="steelblue", alpha=0.75)
        ax.axvline(med, color="k", lw=1.2, label=f"median={med:.2f}°")
        ax.axvspan(
            lo68, hi68, alpha=0.15, color="k", label=f"68% [{lo68:.1f}, {hi68:.1f}]°"
        )
        ax.set_xlabel("T0 (°)")
        ...
        if full_path is not None:
            ...
            idx_f = torch.multinomial(
                w_f, settings["n_predictive_curves"], replacement=True
            )
            samp_f = (
                m_f[0, idx_f, :]
                + s_f[0, idx_f, :]
                * torch.randn(settings["n_predictive_curves"], m_f.shape[-1])
            ).clamp(0.0, 1.0)
            ...
            t_grid_t = torch.tensor(t_vals[:grid_steps], dtype=torch.float32).unsqueeze(
                1
            )
